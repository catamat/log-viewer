...
func main() {
	...
	app.Get("/log-viewer", func(c *fiber.Ctx) error {
		return c.Type("html").SendString(renderIndexHTML())
	})

	app.Get("/log-viewer/favicon.svg", func(c *fiber.Ctx) error {
		c.Set(fiber.HeaderCacheControl, "public, max-age=86400")
		return c.Type("svg").SendString(appBarIconSVG)
	})
...
<head>
  ...
  <link rel="icon" href="/log-viewer/favicon.svg" sizes="any" type="image/svg+xml" />
